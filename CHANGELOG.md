...
*Exam run-ins.* Each exam now gets **two dedicated single-exam days** — an "eve" (D-1) and a "lead-in" — off-limits to other courses' prep, homework, projects and regular study. Exams are grouped into clusters (chains within `FINALS_STRETCH_MAX_SPAN` of each other), so a midterm week and finals week are handled separately. Within a cluster of ≥2 exams, the lead-in days are the block of free days just before the cluster's earliest eve, handed out **in exam order** — soonest exam gets the earliest lead-in day. So a MMW 11/2 / MATH 11/4 / DSC 11/6 cluster produces: 10/29 MMW, 10/30 MATH, 10/31 DSC (lead-ins), then 11/1 MMW, 11/3 MATH, 11/5 DSC (eves), with exam days and 11/7 clear. A lone exam gets its eve plus the calendar day before it. Time per exam is its estimated (difficulty-driven) hours: the eve carries ~60%, capped at 7h/day (`EXAM_EVE_CAP`); only a genuinely large demand spills onto earlier shared days at 3.5h.
...
**Validation:** 42 unit tests pass (`projects.test.js` adds 4: early start, finish buffer, homework still window-bound, keeps going through finals; `examPrepass.test.js` covers the eve + lead-in structure, lead-ins in exam order, two dedicated days per exam with spill only when large, and a lone midterm's two-in-a-row). `npm run build` succeeds. Browser-verified against the Fall 2026 finals: the run-up reads 10/29 MMW, 10/30 MATH, 10/31 DSC, 11/1 MMW, 11/3 MATH, 11/5 DSC — each day one course — with exam days and 11/7 clear.
