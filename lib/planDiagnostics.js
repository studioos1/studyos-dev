...
export function computePlanDiagnostics(data) {
  ...
  const items = r.summaryItems.map(it => {
    ...
    return {
      ...
      isProject: d.kind === "project", // buildItemDemand's own finer kind ("homework"/"project"/"study"),
                                        // distinct from source.type above — needed to tell a project
                                        // apart from regular homework, which source.type can't do.
      forced: !!d.forced,
      title: it.title,
