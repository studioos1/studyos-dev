...
// own vercel.json cron entry — see that route's comment for why) and independently exposed via
...
export async function runNotifyUrgentItems(supabase, rows, today) {
  ...
  for (const row of rows || []) {
    const d = row.data;
    const p = d?.profile;
    if (!d?.onboarded || !p || p.remindersOn === false) { results.push({ user: row.user_id, skipped: "not eligible" }); continue; }
