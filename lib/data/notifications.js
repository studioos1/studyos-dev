...
import { applyTermScopedPatch } from "./terms";
...
export async function runNotifyUrgentItems(supabase, rows, today) {
  ...
  for (const row of rows || []) {
    ...
    try {
      ...
      // This write happens entirely server-side (service-role client, no upd() callback to route
      // through) — applyTermScopedPatch is the same pure mirroring logic the client uses, reused
      // here so notifications lands in the current term's own isolated copy too, not just the flat
      // mirror, even though this cron run never touches components/App.jsx at all.
      await supabase.from("user_data").update({ data: applyTermScopedPatch(d, patch) }).eq("user_id", row.user_id);
