...
export const EP={
  ...
  // The one Getting Started checklist item (components/shared/HelpDrawer.jsx, lib/help.js) with no
  // honest auto-detect signal — every schedule field already has a sane default whether or not the
  // student ever opened Preferences, so there's no data-driven way to tell "reviewed" from "never
  // touched." Manually toggled from the drawer itself instead, but still persisted here (not just
  // component state) so it survives a reload rather than silently unchecking itself.
