...
export const EP={
  ...
  // The two Getting Started checklist items (components/shared/HelpDrawer.jsx, lib/help.js) with no
  // honest auto-detect signal — every schedule field already has a sane default whether or not the
  // student ever opened Preferences, and leaving an AI difficulty/hours estimate unchanged is just
  // as legitimate an outcome as adjusting it, so neither can be read off the data alone. Manually
  // toggled from the drawer itself instead, but still persisted here (not just component state) so
  // they survive a reload rather than silently unchecking themselves.
  helpScheduleReviewed:false,
  helpDifficultyReviewed:false,
};
...
// all its academic data, study plans, grades, user behaviour. ALL... no cross-talking." courses/
// assignments/exams were already correctly isolated (termId/courseId-tagged, filtered by
// termCourseIds everywhere) — grades live ON those records, so they're already isolated too. The
// real, confirmed gap: studyPlan/completionLog/pomodoroLogs/gymLogs/dailyLogs/adhoc/briefCache/
// briefPeriod/quarterPlan/planStale/notifications/lastSyllabusSync were flat, global stores shared
// across every term (see CLAUDE.md's former "biggest open gap" note) — switching which term is
// current never actually changed what any of these showed, so e.g. a syllabus-sync record from
// Term A kept showing while Term B was current, exactly the reported bug.
//
// Fixed by extending this app's own existing "mirror pattern" (already used for
// profile.termStart/termEnd/schoolName/etc, per CLAUDE.md) to these fields too: each one becomes a
// REAL, isolated field on every term object (data.terms[i].studyPlan etc — the actual source of
// truth, this list is what gets reset to empty on a fresh/reset term), while the flat top-level
// copies most of this codebase already reads/writes directly (Today.jsx, Week.jsx, the planner,
// the SMS cron routes, ~50 call sites total) become a live MIRROR of whichever term is current —
// kept in sync by applyTermScopedPatch (lib/data/terms.js), which every upd() call now routes
// through. This is why almost none of those ~50 call sites needed to change: they still read/write
// the same flat data.studyPlan etc as always, it's just genuinely per-term underneath now, not a
// shared blob. `history` is excluded — audited and confirmed completely unused/dead (nothing in
// the app reads or writes data.history; only window.history, the unrelated browser API).
export const TERM_SCOPED_KEYS=["studyPlan","completionLog","pomodoroLogs","gymLogs","dailyLogs","adhoc","briefCache","briefPeriod","quarterPlan","planStale","notifications","lastSyllabusSync"];
export const TERM_DATA_DEFAULTS={studyPlan:{weeks:{}},completionLog:[],pomodoroLogs:[],gymLogs:[],dailyLogs:[],adhoc:[],briefCache:null,briefPeriod:null,quarterPlan:null,planStale:false,notifications:[],lastSyllabusSync:null};
