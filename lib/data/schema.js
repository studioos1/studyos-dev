...
export const EP={
  ...
  funWD:1.5,funWE:4,gymDays:GYM0,gymStretch:30,gymDrive:10,chores:[],remindersOn:true,termName:"",termStart:"",termEnd:"",
  ...
  // Onboarding tour (first-time guided walkthrough of the live app). tourOfferedAt is the trigger
  // gate — set the moment it's shown (finished OR skipped), so it never auto-pops again; a
  // one-time backfill sets it silently for already-onboarded accounts so existing users aren't
  // surprised by it after this ships (see backfillTourOfferedIfNeeded in lib/data/tour.js).
  // tourCompletedAt is purely informational — set only if actually finished, not on skip.
  tourOfferedAt:null,
