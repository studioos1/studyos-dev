...
export const EP={
  ...
  // Browser notifications — per-type toggles (default ON — matches the pattern below for SMS,
  // and matches what the old single remindersOn switch defaulted to). Browser permission itself
  // (the OS/browser-level grant, not these) is tracked live via Notification.permission, not
  // stored here. notifyBrowserPriorities: the once-a-day "today's priorities" notification (due
  // dates within 2 days, exams within 5) — has a server-side counterpart too
  // (app/api/cron/notify-urgent-items) that writes the same content into the bell log even if the
  // browser was never open, gated on this same toggle. notifyBrowserSessions: "time to start
  // studying" nudges at each planned session's actual clock time. notifyBrowserBreaks: break
  // start/end nudges — covers BOTH the passive schedule-driven check and Today's own Focus Timer
  // chime (components/Today.jsx), which used to ignore remindersOn entirely; unified under one
  // toggle so "turn off break reminders" actually turns off every break reminder.
  ...
  // who turns SMS on presumably wants all three; they opt individual ones back out). phone (above)
