...
import { repairTermLinkageIfNeeded, migrateLegacyTermIfNeeded, datesOverlap, canDeleteTerm } from "./terms";
...
describe("canDeleteTerm", () => {
  ...
  const completed = { id: "term_done", status: "completed" };
  ...
  it("blocked: a completed term", () => {
    const r = canDeleteTerm(completed, []);
    expect(r.deletable).toBe(false);
