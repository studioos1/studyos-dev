...
export function migrate(d) {
  ...
  if (d.profile) {
    ...
    // One-time carry-forward: the single remindersOn master switch was split into three per-type
    // browser toggles (notifyBrowserPriorities/Sessions/Breaks — schema.js). Everywhere those are
    // read, "unset" already means "on" (the same convention every other toggle in this app uses),
    // which is exactly right for an account that never touched remindersOn (it defaulted true) —
    // no write needed. But an account that had explicitly turned remindersOn OFF would otherwise
    // silently get all three back on the moment they're read as unset; carry that off-state
    // forward once so it isn't lost. Guarded on the new keys being genuinely unset, so this only
    // ever fires the one time per account.
    if (d.profile.remindersOn === false && d.profile.notifyBrowserPriorities === undefined) {
      d.profile.notifyBrowserPriorities = false;
      d.profile.notifyBrowserSessions = false;
      d.profile.notifyBrowserBreaks = false;
    }
  }
