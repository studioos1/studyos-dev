...
// calendar (termAsProfile) instead of whichever term is current, and studyPlan/quarterPlan read
// from its own isolated copy (TERM_SCOPED_KEYS) instead of the flat top-level mirror. For the
// current term this is equivalent to `data` itself (every one of those fields is already kept in
// sync with it) — this only actually diverges for a non-current term, which is the whole point:
// the term-viewer's "show me this OTHER term's plan/calendar, without touching what's live" path.
...
export function projectTermForPlanning(data,term,school){
  ...
  return{
    ...
    studyPlan:term.studyPlan!==undefined?term.studyPlan:TERM_DATA_DEFAULTS.studyPlan,
    quarterPlan:term.quarterPlan!==undefined?term.quarterPlan:TERM_DATA_DEFAULTS.quarterPlan,
  };
}
