...
import { uid, TERM_SCOPED_KEYS, TERM_DATA_DEFAULTS } from "./schema";
...
// Scopes courses/assignments/exams to just one term before handing off to the planner. Without
// this, the planner (both the deadline-driven scheduler and the regular-study fallback, which
// iterates data.courses directly) would consider EVERY course/assignment/exam ever created —
// including years-old archived terms kept for history, and any upcoming term prepped in advance —
// mixing all of it into today's real schedule. freeSlots() also reads data.courses for class-time
// blocking, so scoping it here via the same data object fixes both at once. Defaults to whichever
// term is current (all existing call sites) — pass an explicit `term` to scope to a different one
// instead (the term-viewer's "run a plan for the term I'm looking at, not necessarily Current").
// If no term exists yet (e.g. before the legacy migration has run), falls back to unscoped data
// rather than breaking a fresh/pre-migration install.
export function termScopedForPlanning(data,term){
  const targetTerm=term||getActiveTermAndSchool(data).term;
  if(!targetTerm)return data;
  const scopedCourses=data.courses.filter(c=>c.termId===targetTerm.id);
  const scopedCourseIds=new Set(scopedCourses.map(c=>c.id));
  return{
    ...data,
    courses:scopedCourses,
    assignments:data.assignments.filter(a=>scopedCourseIds.has(a.courseId)),
    exams:data.exams.filter(e=>scopedCourseIds.has(e.courseId)),
  };
}
// Builds a profile-shaped object ({termStart,termEnd,schoolName,schoolAddress,schoolType,
// collegeCalendar}) from a single term (+ its school) — the exact shape
// syncActiveTermToProfilePatch mirrors onto the real profile for whichever term is current. Every
// existing consumer of "the active term's dates/calendar" (getTermRange, isFin, isHol, getQ) only
// ever reads a profile-shaped object, never `data` itself, so building this for an arbitrary term
// lets all of them work unchanged against a term that ISN'T current — the term-viewer's read path,
// with zero changes needed to any of those functions.
export function termAsProfile(term,school){
  if(!term)return null;
  return{
    termName:term.name,termStart:term.start||"",termEnd:term.end||"",
    schoolName:school?.name||"",schoolAddress:school?.address||"",schoolType:term.type||school?.schoolType||"quarter",
    collegeCalendar:(term.start&&term.end)?{
      quarters:[{name:term.name,start:term.start,end:term.end}],
      holidays:term.holidays||[],source:term.source||null,fetchedAt:term.fetchedAt||null,
    }:null,
  };
}
// Projects a whole `data` object onto a single term for planning/rendering purposes — courses/
// assignments/exams scoped to it (termScopedForPlanning), profile swapped to ITS OWN dates/
// calendar (termAsProfile) instead of whichever term is current, and EVERY TERM_SCOPED_KEYS field
// (studyPlan, quarterPlan, completionLog, pomodoroLogs, gymLogs, dailyLogs, briefCache/briefPeriod,
// planStale, notifications, lastSyllabusSync, adhoc) read from its own isolated copy instead of the
// flat top-level mirror — not just studyPlan/quarterPlan (an earlier version of this only projected
// those two, which left Today.jsx seeing a mismatched mix once it started using this too: the viewed
// term's own assignments/schedule against Current's completionLog/notifications/etc). For the
// current term this is equivalent to `data` itself (every one of those fields is already kept in
// sync with it) — this only actually diverges for a non-current term, which is the whole point: the
// term-viewer's "show me this OTHER term's plan/calendar, without touching what's live" path.
// Read-only — for writing a plan back into a non-current term see applyTermScopedPatch's
// `targetTermId` param.
export function projectTermForPlanning(data,term,school){
  if(!term)return data;
  const scoped=termScopedForPlanning(data,term);
  const scopedFields={};
  TERM_SCOPED_KEYS.forEach(k=>{scopedFields[k]=term[k]!==undefined?term[k]:TERM_DATA_DEFAULTS[k];});
  return{
    ...scoped,
    ...scopedFields,
    profile:{...data.profile,...termAsProfile(term,school)},
  };
}
// The single choke point that keeps every term-scoped field's flat top-level mirror — read/written
// directly by ~50 call sites across the app (Today.jsx, Week.jsx, the planner, the SMS cron
// routes, ...), none of which needed to change for this — in sync with whichever term is actually
// current, and routes any direct write to one of those fields into that term's own isolated copy.
// See TERM_SCOPED_KEYS (lib/data/schema.js) for the full list and why. components/App.jsx's upd()
// is a two-line wrapper around this; kept here, pure, so it's independently testable. Two cases:
//  1. The patch touches `terms` itself (a status change, term create/reset/delete) — the current
//     term may now be a different one (or the same one with different nested data), so every
//     mirrored field is unconditionally re-derived from whichever term is current AFTER the patch.
//  2. A normal flat write to one of the scoped fields (Today.jsx completing a task, the planner
//     replanning, a syllabus sync recording lastSyllabusSync, ...) — mirrored into the current
//     term's own copy too, so it's genuinely isolated, persisted data, not just a flat value that
//     would look like it belongs to whichever term happens to be current later.
// Optional 3rd arg `targetTermId`: routes a scoped-key write into that SPECIFIC term instead of
// whichever one is current — the term-viewer's "run/clear a plan for the Upcoming term I'm looking
// at" path, where the write must land in that term's own isolated copy and NOT touch Current's live
// flat fields at all. Omit it (every pre-existing call site) for the exact original behavior.
export function applyTermScopedPatch(prevData,patch,targetTermId){
  const merged={...prevData,...patch};
  if("terms" in patch){
    const{term}=getActiveTermAndSchool(merged);
    if(term)TERM_SCOPED_KEYS.forEach(k=>{merged[k]=term[k]!==undefined?term[k]:TERM_DATA_DEFAULTS[k];});
    return merged;
  }
  const scopedKeys=Object.keys(patch).filter(k=>TERM_SCOPED_KEYS.includes(k));
  if(!scopedKeys.length)return merged;
  const{term:currentTerm}=getActiveTermAndSchool(prevData);
  const targetingNonCurrent=targetTermId&&currentTerm&&targetTermId!==currentTerm.id;
  const term=targetingNonCurrent?(prevData.terms||[]).find(t=>t.id===targetTermId):currentTerm;
  if(!term)return merged; // no current/target term (e.g. pre-onboarding, or a stale/deleted target) — nothing to mirror into
  const scopedPatch={};
  scopedKeys.forEach(k=>scopedPatch[k]=patch[k]);
  merged.terms=(merged.terms||prevData.terms).map(t=>t.id===term.id?{...t,...scopedPatch}:t);
  if(targetingNonCurrent){
    // Writing into a term that ISN'T current — undo the flat top-level mirror the initial spread
    // above just applied (those fields are Current's live state and must stay untouched); keep
    // only the nested write into the target term's own copy, just made above.
    scopedKeys.forEach(k=>{merged[k]=prevData[k];});
  }
  return merged;
}

// One-time migration: seeds each term's own isolated copy of the scoped fields (TERM_SCOPED_KEYS)
// the first time this runs on an account created before real per-term isolation existed. The
// CURRENT term (if any) adopts whatever's sitting in the old flat top-level fields — the honest
// assumption, since that flat data really was generated while it was active — and every OTHER term
// starts from a genuinely empty default, never inheriting anything (no date-range guessing needed
// anymore, unlike the old scrubTermSchedule this replaces — real isolation makes that moot).
// Idempotent: a term already carrying its own studyPlan field (impossible to have been left
// unset by TERM_DATA_DEFAULTS once migrated) is left completely alone, so this runs safely on
// every load without re-clobbering anything.
export function migrateTermDataIsolationIfNeeded(data){
  const terms=data.terms||[];
  if(!terms.length)return null;
  if(terms.every(t=>t.studyPlan!==undefined))return null; // already migrated
  const{term:currentTerm}=getActiveTermAndSchool(data);
  const fixedTerms=terms.map(t=>{
    if(t.studyPlan!==undefined)return t;
    if(currentTerm&&t.id===currentTerm.id){
      const adopted={};
      TERM_SCOPED_KEYS.forEach(k=>{adopted[k]=data[k]!==undefined?data[k]:TERM_DATA_DEFAULTS[k];});
      return{...t,...adopted};
    }
    return{...t,...TERM_DATA_DEFAULTS};
  });
  return{terms:fixedTerms};
}
