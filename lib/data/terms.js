...
export function applyTermScopedPatch(prevData,patch){
  ...
  const{term}=getActiveTermAndSchool(prevData);
  if(!term)return merged; // no current term yet (e.g. pre-onboarding) — nothing to mirror into
  const scopedPatch={};
