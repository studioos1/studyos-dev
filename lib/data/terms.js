...
// The ONE place that reconciles studyPlan/completionLog/pomodoroLogs against a term — those three
// stores are flat and date-keyed, not termId-tagged (see the "Multi-school/multi-term system"
// note in CLAUDE.md), so "this term's schedule" has to be reconstructed by date-range + courseId
// rather than filtered directly. Used by every place that needs a term's schedule/history
// data-scrubbed alongside its courses/assignments/exams: School Info's per-term Reset and Delete,
// new-term creation (so a fresh term doesn't inherit whatever was on its dates from an earlier
// term), and Academics' own "Reset academic data" (Courses → Update Syllabus) — that last one used
// to be a separate, parallel implementation that only cleared courses/assignments/exams and never
// called this, a real, reported bug: "after Reset academic data... the calendar still shows study
// plans." Consolidated here so there is only ever one place this logic can drift out of sync.
// t = a term object ({start,end}); termCourseIds = the Set of course ids that belonged to it —
// pass an empty Set (not that term's own, since it has none yet) when scrubbing for a brand-new
// term, so only the DATE-based half of the scrub applies.
export function scrubTermSchedule(data,t,termCourseIds){
