...
// Gym per-day duration — Preferences → Gym & Fun asks for start time + duration (a select, same
// idiom as meal duration), not start+end, so gd.e is always DERIVED as gd.s + duration rather than
// typed directly. Real requested UX fix: two separate time pickers for one session made the
// student do the subtraction themselves to know how long a session actually was.
export const GYM_DUR_OPTIONS=[30,45,60,75,90,120];
export const CC=[
