...
// The current "daily briefing" period, anchored to 8am LOCAL time — not midnight, and NOT the
// app's build version (that was the real bug: the cached brief invalidated on every rebuild,
// firing a real paid AI call just from reloading the page, unrelated to whether the day's actual
// facts had changed). Before 8am, we're still in YESTERDAY's period — matches a student's day
// conceptually starting in the morning, not at 12:00am while they might still be up studying.
// Returns the ISO date the period STARTED on, e.g. "2026-09-15" names the period
// 2026-09-15T08:00 through 2026-09-16T08:00 (local time). `now` is injectable for testing.
export function briefPeriodStart(now){
...
export function sundayOf(d){const x=new Date(d);x.setHours(0,0,0,0);x.setDate(x.getDate()-x.getDay());return x;}
