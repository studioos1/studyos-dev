...
import { weekStartOf, catchUpDays, todayPassedBlocks, catchUpMarkComplete, CATCHUP_DAYS, isItemScheduled } from "./weeks";
...
// minutes-from-midnight) default to 10:00-11:00 but can be overridden per entry.
function mkData(entries) {
  const weeks = {};
  entries.forEach(({ date, id, completed, s = 600, e = 660 }) => {
    const ws = weekStartOf(date);
    weeks[ws] = weeks[ws] || { days: {} };
    weeks[ws].days[date] = [
      ...(weeks[ws].days[date] || []),
      { id, s, e, label: `Session ${id}`, kind: "study", courseId: "c1", course: "MATH 180A", completed: !!completed, source: {} },
    ];
  });
