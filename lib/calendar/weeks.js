...
// Study/homework/project sessions genuinely crossing a study-start/break-start/block-end
// boundary RIGHT NOW — the passive, schedule-driven reminder App.jsx's notification watcher
// fires from (real reported gap: notifications only ever fired for a session the student had
// already manually started; StudyOS never proactively said "10:00 MATH 180A — start studying").
// Bounded to REMINDER_WINDOW minutes after the boundary, not open-ended, so a block from hours
// ago never fires a stale flurry the moment the app happens to be opened. `now` is an optional
// override (tests only), same pattern as todayPassedBlocks/briefPeriodStart.
const REMINDER_WINDOW=10; // minutes
const STUDY_REMINDER_KINDS=new Set(["study","homework","project"]);
export function scheduleReminders(data,now){
  const today=iso();
  const d=now?new Date(now):new Date();
  const nowMin=d.getHours()*60+d.getMinutes();
  const focusMins=(+data.profile?.focusMins)||25,breakMins=(+data.profile?.breakMins)||5;
  const within=(boundary)=>nowMin>=boundary&&nowMin<boundary+REMINDER_WINDOW;
  const out=[];
  realDayBlocks(data,today).filter(b=>STUDY_REMINDER_KINDS.has(b.kind)&&!b.completed).forEach(b=>{
    const studyStart=t2m(b.time);
    const breakStart=studyStart+focusMins;
    const blockEnd=breakStart+breakMins;
    const label=b.course?`${b.course} — ${b.task}`:b.task;
    if(within(studyStart))out.push({key:`${b.id}|start`,title:"📚 Time to start studying",body:label});
    else if(within(breakStart))out.push({key:`${b.id}|break`,title:"☕ Time for a break",body:`${label} — take ${breakMins} min.`});
    else if(within(blockEnd))out.push({key:`${b.id}|done`,title:"💪 Break's over",body:`Back to ${label}, or mark it done.`});
  });
  return out;
}
