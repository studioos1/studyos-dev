import { iso, m2t, du } from "@/lib/time";
...
// now" — same three conditions as Prog.jsx's own `tasks`/`catchDays`, but without building the
// display-formatted lists. Used to gate the evening report-complete nudge (App.jsx): the nudge
// should never fire on a day with genuinely nothing to report.
export function hasCheckInWork(data){
  const dueOrOverdue=(data.assignments||[]).some(a=>a.status!=="done"&&a.dueDate&&du(a.dueDate)<=2);
  const examPrepWindow=(data.exams||[]).some(e=>{const d=du(e.date);return d>=0&&d<=e.prepDays;});
  return dueOrOverdue||examPrepWindow||catchUpDays(data).length>0;
}
