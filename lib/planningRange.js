...
//
// Optional `term`: scopes to that term specifically instead of resolving whichever one is current —
// the term-viewer's "plan for the term I'm looking at" path. `data.profile` is expected to already
// reflect that same term (via projectTermForPlanning) when this is passed; only the course/
// assignment/exam scoping below needs telling explicitly, since termScopedForPlanning would
// otherwise re-resolve current from data.terms and undo the caller's own projection.
export function planningRange(data, term) {
