...
// schedule itself (.github/workflows/scheduled-reminders.yml — moved here after Vercel Cron Jobs
// turned out to be silently unavailable on the Hobby plan) is a static UTC time, so the actual
// fire time drifts by an hour across a DST transition until the UTC value is manually adjusted
// twice a year — this zone (used for what DATE counts as "today" when building the message) stays
// DST-correct via Intl regardless, so a drifted fire time still sends the right day's content,
// just not exactly at 8:30/8:00 on the two DST-transition days each year.
