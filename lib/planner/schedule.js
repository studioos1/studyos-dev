...
export function buildExamPrepPlan(dateStrs,itemState,gapsByDay){
  ...
  const reserve=(exId,d)=>{leadInOf[exId]=d;used.add(d);capLeft[d]=Math.min(freeMin(d),EXAM_EVE_CAP);};
  ...
  // Eve carries ~60% (capped at EXAM_EVE_CAP), the lead-in day takes the rest, the eve mops up
  // any remainder, and only a genuinely large demand spills onto earlier shared days.
  ...
  for(const ex of examsWithEve){
    ...
    if(leadIn)put(leadIn,EXAM_EVE_CAP);
    ...
    Object.entries(alloc).forEach(([d,m])=>{
      ...
      if((d===eve||d===leadIn)&&m>=15)reservedExamDays[d]=ex.id; // this dedicated day is genuinely in use
    });
  }
