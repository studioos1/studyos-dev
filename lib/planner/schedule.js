...
// day in isolation). Implements every agreed rule together: session length, real priority
// scoring, primary/secondary with minimize-switching, 15-min topic buffer, 1-day due-date
// buffer, energy-peak time-of-day placement, and a completion guarantee with early risk warnings
// — validated as a standalone prototype before this integration (see project history).
// ══════════════════════════════════════════════════════════════════════════════════════════

// The combined study+break chunk length used for grid-aligned scheduling — derived from the same
// focusMins/breakMins the Pomodoro timer uses, instead of a separate "session preset" that used
// to duplicate the same concept with its own 3 fixed choices (sessionPreset, removed). Rounded to
// the nearest 15 so placed blocks always land on the scheduling grid — the focus/break dropdowns
// offer finer-than-15 resolution now, but a 25-minute study-only chunk (say, 20 focus + 5 break)
// wouldn't end on the 15-min grid the way a 30-minute combined block always does.
export function presetLenFor(profile){
  const sum=(+profile.focusMins||25)+(+profile.breakMins||5);
  return Math.max(15,Math.round(sum/15)*15);
}
...
// energyPeakTime: an "HH:MM" clock time (replaces the old 3-bucket morning/afternoon/evening
// enum) — classified into the same three broad windows dayWindows() uses (before noon / noon-5pm
// / after 5pm), then that window is placed first in the priority order, so scheduling still
// prioritizes whichever part of the day the student is actually sharpest at, just picked with
// real time-of-day precision instead of a coarse bucket.
export function windowOrderFor(energyPeakTime){
  const m=t2m(energyPeakTime);
  const bucket=!Number.isFinite(m)?"morning":m<720?"morning":m<1020?"afternoon":"evening";
  if(bucket==="afternoon")return["afternoon","evening","morning"];
  if(bucket==="evening")return["evening","afternoon","morning"];
...
export function buildExamPrepPlan(dateStrs,itemState,gapsByDay){
  ...
  // Forced exams claim the shared "earlier days" overflow pool first (the spill loop just below,
  // when an exam's own eve+lead-in aren't enough) — a real bug otherwise: `forced` only widened
  // an exam's own start window (windowCapOf above), it never moved the exam earlier in THIS
  // queue, so a forced exam due later than another exam was still served after it and could
  // still lose the leftover shared capacity to a non-forced exam that got there first. Sort is
  // stable, so due-date order is preserved within each forced/non-forced group — this only moves
  // forced exams to the front as a group, it doesn't reorder anything else.
  const examsWithEve=exams.filter(ex=>eveOf[ex.id])
    .sort((a,b)=>(b.forced?1:0)-(a.forced?1:0));
      ...
      for(const wg of gaps.map(g=>({s:Math.max(g.s,winStart),e:Math.min(g.e,winEnd)})).filter(g=>g.e-g.s>=15)){
        ...
        while(remaining>=15){
          ...
          const label=item.kind==="homework"
            ...
            :`${item.courseName} — ${item.title} exam prep (${daysFrom(dateStr,item.dueDate)===0?"today!":`${daysFrom(dateStr,item.dueDate)}d left`})`;
          placed.push({
...
export function planDayV2(dateStr,itemState,data,gaps,seqRef,examPrep){
  ...
  const presetLen=presetLenFor(profile);
  ...
  const winOrder=windowOrderFor(profile.energyPeakTime);
  const blocks=[];
  let totalPlaced=0;
  ...
  Object.entries(examPrep?.byDate?.[dateStr]||{}).forEach(([examId,minutes])=>{
    ...
    const label=daysLeft===1
      ?`${it.courseName} — ${it.title} — final review`
      :`${it.courseName} — ${it.title} exam prep (${daysLeft}d left)`;
    ...
    if(it&&surplus>=15){
      ...
      const{blocks:exBlocks,placed}=placeMinutes(gaps,surplus,presetLen,winOrder,winMap,
        {courseId:it.courseId,courseName:it.courseName,source:it.source,kind:"study",
         label:`${it.courseName} — ${it.title} exam prep (${daysLeft}d left)`},dateStr,seqRef,now);
      blocks.push(...exBlocks);
      it.remainingMinutes-=placed;
      totalPlaced+=placed;
    }
  ...
  if(totalPlaced<DAILY_CAP&&!reservedExamId&&!leadInExamId){
    // Forced projects go first — `forced` was never consulted here at all before (unlike
    // homework's priority=1e9), so on a day where multiple projects compete for the same
    // DAILY_CAP budget, a forced project could still lose out to a non-forced one processed
    // earlier in this loop and hit the `if(totalPlaced>=DAILY_CAP)break` below before its own
    // turn. Stable sort — due-date order otherwise unchanged.
    const projects=itemState.filter(it=>it.kind==="project"&&it.remainingMinutes>0&&it.dueDate)
      .sort((a,b)=>(b.forced?1:0)-(a.forced?1:0)||a.dueDate.localeCompare(b.dueDate)); // forced, then soonest-due
...
export function preflightRiskCheck(dateStrs,itemState,gapsByDay,examPrep){
  ...
  // Forced items claim shared capacity first, ahead of even due-date order — matching the real
  // planner's own behavior (homework's priority=1e9, and now exams'/projects' forced-first
  // ordering above), so this preflight estimate doesn't predict a shortfall the actual planner
  // wouldn't produce for a forced item. It was previously entirely unconsulted here.
...
export function planHorizon(dateStrs,data,gapsByDayFn,userEditedByDate){
  ...
  const summaryItems=original.map(orig=>{
    ...
    // forced carried through so the caller can call out what happened to a specifically-
    // prioritised item by name, instead of only an undifferentiated top-N shortfall list.
    return{id:orig.id,title:orig.title,courseName:orig.courseName,forced:!!orig.forced,
