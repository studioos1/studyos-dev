...
// ── Exam prep pre-pass ──────────────────────────────────────────────────────────────────────
// Exam prep is decided GLOBALLY before day-by-day planning, so it can be back-loaded (packed
// toward the exam) and prioritised across exams — things a greedy per-day pass can't do well.
// Rules it implements: (B) fill from D-1 backward; (C) the soonest exam claims scarce near-days
// first, harder exams breaking ties; (D) a per-exam daily cap + spread over ≥2 days (≥3 for
// high-stakes); (F) never on an exam day; (A) the exam owns its RUN-IN — its last EXAM_RUN_IN_DAYS
// eligible days (D-1 and D-2) are exclusively its, so no other course's prep or study lands there
// and the exam gets two solid days in a row; (E) a "finals stretch" span during which Tier-2
// regular study is suppressed. Because each exam claims its run-in nearest-first, the run-in days
// across a finals cluster end up sequenced in exam order.
export const EXAM_DAILY_CAP=210;          // one exam's max minutes on an ordinary prep day (3.5h)
export const EXAM_EVE_CAP=420;            // …raised to 7h on each of the exam's own run-in days (D-1, D-2)
export const EXAM_RUN_IN_DAYS=2;          // how many days before the exam are its exclusive run-in
...
export function buildExamPrepPlan(dateStrs,itemState,gapsByDay){
  ...
  let finalsStretch=null;
  if(exams.length>=2){
    const first=exams[0].dueDate,last=exams[exams.length-1].dueDate;
    if(daysFrom(first,last)<=FINALS_STRETCH_MAX_SPAN)finalsStretch={start:shiftDate(first,-1),end:last};
  }

  // Per-day room for exam prep: the day's real free minutes, ceilinged at one full study-day
  // (DAILY_CAP) so several exams sharing a day can't overfill it. Shared across exams in the loop
  // below — the soonest exam (processed first) claims scarce near-days first (rule C).
  const DAILY_CAP=300;
  ...
  const runInOf={};       // examItemId -> [D-1, D-2] : its exclusive run-in days
  const claimedDays=new Set(); // every run-in day claimed so far — off-limits to every other exam (rule A)
  const shortfalls=[];

  for(const ex of exams){
    const windowCap=ex.forced?Infinity:ex.startWindowDays;
    const eligible=dateStrs.filter(d=>{
      const o=daysFrom(d,ex.dueDate);
      // In window, strictly before the exam, not on any exam day (rule F), and not another
      // (nearer, already-processed) exam's exclusive run-in day (rule A).
      return o>=1&&o<=windowCap&&!examDates.has(d)&&!claimedDays.has(d);
    }).sort((a,b)=>b.localeCompare(a)); // D-1 first (rule B — back to front)

    if(!eligible.length){shortfalls.push({id:ex.id,shortfallMin:ex._demand});continue;}
    // Run-in = the eve, plus each preceding CALENDAR day (up to EXAM_RUN_IN_DAYS total) that is
    // still a free prep day — so it's genuinely "N days in a row", never a gap-jump. In a tight
    // finals cluster the day before the eve is often the previous exam's day, so a later exam may
    // get just its eve; the first exam in the cluster gets the full consecutive run-in.
    const eligibleSet=new Set(eligible);
    const runIn=[eligible[0]];
    for(let k=1;k<EXAM_RUN_IN_DAYS;k++){
      const prev=shiftDate(runIn[runIn.length-1],-1);
      if(!eligibleSet.has(prev))break;
      runIn.push(prev);
    }
    runIn.forEach(d=>{
      claimedDays.add(d);
      capLeft[d]=Math.min(freeMin(d),EXAM_EVE_CAP); // an exclusive run-in day may be packed past the shared-day ceiling
    });
    runInOf[ex.id]=runIn;

    const highStakes=(ex.weight!=null&&+ex.weight>=25)
      ||ex.effectiveDifficulty==="High"||ex.effectiveDifficulty==="Very High";
    // A run-in day may be packed to a full study-day (EXAM_EVE_CAP); ordinary prep days stay at
    // the lighter EXAM_DAILY_CAP.
    const dayCap=d=>runIn.includes(d)?EXAM_EVE_CAP:EXAM_DAILY_CAP;
    // Minimum number of distinct days to touch (rule D / G): at least the run-in, 3 for a
    // high-stakes exam, more only if the daily cap alone forces it. Never more than are available.
    const minSpread=Math.min(
      eligible.length,
      Math.max(runIn.length,highStakes?3:2,Math.ceil(ex._demand/EXAM_EVE_CAP)),
    );

    // Greedy back-load (rule B): fill the eve as full as the day allows, then D-2, D-3, … — the
    // eve always carries the most. Hold 15 min back for each still-unfilled required day so the
    // minimum spread is still met.
    let remaining=ex._demand;
    const alloc={};
    let daysStillRequired=minSpread;
    for(const d of eligible){
      if(remaining<15)break;
      daysStillRequired=Math.max(0,daysStillRequired-1);
      const room=Math.min(dayCap(d),capLeft[d],remaining-15*daysStillRequired);
      const take=Math.floor(room/15)*15;
      if(take>=15){alloc[d]=take;remaining-=take;capLeft[d]-=take;}
    }
    // Spill anything still left (day-capacity or cap limits hit) onto any eligible day with room.
    if(remaining>=15){
      for(const d of eligible){
        if(remaining<15)break;
        const room=Math.min(dayCap(d)-(alloc[d]||0),capLeft[d],remaining);
        const take=Math.floor(room/15)*15;
        if(take>=15){alloc[d]=(alloc[d]||0)+take;remaining-=take;capLeft[d]-=take;}
      }
    }
  ...
  // Every run-in day an exam actually got time on becomes a reserved, single-exam day (rule A) —
  // planDayV2 keeps homework, projects and regular study off it.
  const reservedExamDays={};
  Object.entries(runInOf).forEach(([examId,days])=>{
    days.forEach(d=>{if(byDate[d]?.[examId])reservedExamDays[d]=examId;});
  });

  return{byDate,reservedExamDays,examDates,finalsStretch,shortfalls};
}
...
export function planDayV2(dateStr,itemState,data,gaps,seqRef,examPrep){
  ...
  const reservedExamId=examPrep?.reservedExamDays?.[dateStr]||null; // this date is exam X's exclusive run-in (rule A)
  ...
  // genuinely due within a day is allowed (rule A).
  ...
  if(courseList.length&&totalPlaced<DAILY_CAP){
    if(blocks.length)consumeTopicBuffer(gaps,blocks[blocks.length-1].e);
    const primary=courseList[0];
    const primaryBudget=Math.min(Math.round(DAILY_CAP*0.65),DAILY_CAP-totalPlaced);
  ...
  // (a project due then still needs work) — but not on an exam's exclusive eve (rule A).
  if(totalPlaced<DAILY_CAP&&!reservedExamId){
  ...
  // ── Tier 2: regular per-course study fills any leftover capacity — but NOT on an exam eve
  //    (rule A) and NOT anywhere inside the finals stretch (rule E). ──
  if(totalPlaced<DAILY_CAP&&!reservedExamId&&!inFinalsStretch){
