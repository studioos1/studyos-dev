...
// first; (D) a per-exam daily cap + spread over ≥2 days (≥3 for high-stakes) so there's still
// spacing; (F) never on an exam day; (A) exclusive-eve flag; (E) a "finals stretch" span during
// which Tier-2 regular study is suppressed. The eve (D-1) is the exception to the daily cap — it
// gets filled as fully as the day allows, because that's the last chance before the exam.
export const EXAM_DAILY_CAP=210;          // one exam's max minutes on a normal prep day (3.5h)
export const EXAM_EVE_CAP=420;            // …but its own eve (D-1) can be packed up to 7h — last chance
...
export function buildExamPrepPlan(dateStrs,itemState,gapsByDay){
  ...
  const exams=itemState
    ...
    .sort((a,b)=>a.dueDate.localeCompare(b.dueDate)); // nearest deadline first (rule C)
  ...
  // (DAILY_CAP) so several exams sharing a day can't overfill it. Each exam's OWN eve gets its
  // ceiling raised to EXAM_EVE_CAP just before that exam is processed (safe — claimedEves means
  // no other exam touches it). Shared across exams in the loop below — the soonest exam
  // (processed first) claims scarce near-days first (rule C).
  const DAILY_CAP=300;
  ...
  const latestDayOf={};   // examItemId -> its latest eligible day (its "eve")
  const claimedEves=new Set(); // an exam's exclusive eve — off-limits to every other exam (rule A)
  const shortfalls=[];
  ...
  for(const ex of exams){
    ...
    const eligible=dateStrs.filter(d=>{
      ...
      // (nearer, already-processed) exam's exclusive eve (rule A).
      return o>=1&&o<=windowCap&&!examDates.has(d)&&!claimedEves.has(d);
    ...
    const eve=eligible[0];
    latestDayOf[ex.id]=eve;
    claimedEves.add(eve); // this exam now owns its eve; later exams can't spill onto it
    capLeft[eve]=Math.min(freeMin(eve),EXAM_EVE_CAP); // and its eve may be packed fuller than a normal day

    const highStakes=(ex.weight!=null&&+ex.weight>=25)
      ||ex.effectiveDifficulty==="High"||ex.effectiveDifficulty==="Very High";
    // Minimum number of distinct days to touch (rule D / G): 2 normally, 3 for a high-stakes
    // exam, more only if the daily cap alone forces it. Never more days than are available.
    const dayCap=d=>d===eve?EXAM_EVE_CAP:EXAM_DAILY_CAP;
    const minSpread=Math.min(
      eligible.length,
      Math.max(highStakes?3:2,Math.ceil(ex._demand/EXAM_EVE_CAP)),
    );
  ...
  // Each exam's eve (claimedEves guarantees no two exams share one) becomes a reserved,
  // single-exam day (rule A) as long as that exam actually got prep time there.
  const reservedEves={};
  Object.entries(latestDayOf).forEach(([examId,d])=>{
    if(byDate[d]?.[examId])reservedEves[d]=examId;
  });

  return{byDate,reservedEves,examDates,finalsStretch,shortfalls};
}
...
export function planDayV2(dateStr,itemState,data,gaps,seqRef,examPrep){
  ...
  const reservedExamId=examPrep?.reservedEves?.[dateStr]||null; // this date is exam X's exclusive eve (rule A)
