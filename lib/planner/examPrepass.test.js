...
describe("exam pre-pass — rule A: a nearer exam's eve is off-limits to a later exam", () => {
  it("three back-to-back finals — each eve carries only its own exam's prep", () => {
    ...
    // The first exam in the cluster also gets the day before its eve as a dedicated run-in day
    // (the later exams can't — that day is the previous exam's date).
    onlyCourse(addDays(examMMW, -2), 1);
  ...
  it("a lone midterm gets two consecutive dedicated run-in days, both off-limits to other work", () => {
    ...
    [addDays(exam, -1), addDays(exam, -2)].forEach((d) => {
      ...
      expect(blocks.length).toBeGreaterThan(0);
      expect(blocks.every(isExamPrep)).toBe(true);                         // only exam prep
      expect(minutesFor(blocks, (b) => b.source?.id === 30)).toBe(0);      // no other course's homework
      expect(minutesFor(blocks, (b) => /regular study/.test(b.label))).toBe(0);
    });
  });
});
