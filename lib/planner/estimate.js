...
// Loose plausible weekly-hours window per COURSE-level difficulty score (1-10, from B-01's
// web-search estimate at /api/course-info) — NOT the same table as STUDY_HOURS_BY_RATING above
// (that's per assignment/exam effort, driven by a Low/Mid/High/Very-High item rating). This is a
// sanity check only: difficultyScore and weeklyHours come back together in one AI response with no
// guarantee the two are actually coherent, especially across two separate research calls for the
// same course (nothing stops one call saying "Heavy, 10h/wk" and the next saying "Heavy, 3h/wk").
// Deliberately wide ranges — real courses vary a lot within a band — this only flags (never
// blocks) a result that's actually incoherent, per the standing "prefer deterministic checks over
// trusting AI in isolation" rule.
export const WEEKLY_HOURS_RANGE_BY_DIFFICULTY=[
  {max:3,  range:[1,5]},  // Light   (difficultyScore 1-3)
  {max:6,  range:[3,8]},  // Medium  (4-6)
  {max:8,  range:[5,12]}, // Heavy   (7-8)
  {max:10, range:[8,18]}, // Intense (9-10)
];
export function expectedHoursRange(difficultyScore){
  const band=WEEKLY_HOURS_RANGE_BY_DIFFICULTY.find(b=>difficultyScore<=b.max)
    ||WEEKLY_HOURS_RANGE_BY_DIFFICULTY[WEEKLY_HOURS_RANGE_BY_DIFFICULTY.length-1];
  return band.range;
}
