...
describe("planHorizon — forced items", () => {
  ...
  // Regression test for a real bug: `forced` only ever elevated priority for regular homework
  // (priority=1e9 in planDayV2's candidates). Exams and projects never consulted `forced` when
  // deciding who gets first claim on genuinely scarce SHARED capacity — an exam/project's own
  // start-window got widened, but its processing order relative to competing exams/projects for
  // the same overflow days stayed due-date-only. So a forced exam due LATER than a competing
  // non-forced exam could still lose the shared capacity to the earlier one and come back short
  // — exactly what was reported live. Both cases below force scarcity tight enough that a
  // shortfall is mathematically guaranteed for ONE of the two competing items, then assert the
  // forced one is the one that's protected, regardless of due-date order.
  it("a forced EXAM gets shared overflow capacity ahead of a non-forced exam due earlier", () => {
    const narrowGaps = makeGapsFn({ dayStart: 540, dayEnd: 600 }); // 60 min/day — tight on purpose
    const dueA = addDays(TODAY, 10), dueB = addDays(TODAY, 12); // close together — same cluster
    const dateStrs = Array.from({ length: 13 }, (_, i) => addDays(TODAY, i));
    const data = {
      profile: PROFILE,
      courses: [{ id: 1, name: "Course A", difficulty: 5, weeklyHours: 0 }, { id: 2, name: "Course B", difficulty: 5, weeklyHours: 0 }],
      assignments: [],
      exams: [
        { id: 2001, courseId: 1, title: "Exam A (earlier, not forced)", date: dueA, status: "not-started", estimatedHours: 8, weight: 10 },
        { id: 2002, courseId: 2, title: "Exam B (later, forced)", date: dueB, status: "not-started", estimatedHours: 8, weight: 10, forced: true },
      ],
    };
    const result = planHorizon(dateStrs, data, narrowGaps, {});
    const combinedShortfall = result.shortfalls.reduce((s, it) => s + it.shortfallHours, 0);
    expect(combinedShortfall).toBeGreaterThan(0); // sanity: this scenario is genuinely capacity-constrained, not a no-op test
    const examB = result.summaryItems.find((it) => it.id === "e_2002");
    expect(examB.fullyCovered).toBe(true); // the forced exam, due LATER, is still fully covered
  });

  it("a forced PROJECT gets daily budget ahead of a non-forced project due earlier", () => {
    const narrowGaps = makeGapsFn({ dayStart: 540, dayEnd: 570 }); // 30 min/day — tight on purpose
    const dueA = addDays(TODAY, 30), dueB = addDays(TODAY, 45);
    const dateStrs = Array.from({ length: 46 }, (_, i) => addDays(TODAY, i));
    const data = {
      profile: PROFILE,
      courses: [{ id: 1, name: "Course A", difficulty: 5, weeklyHours: 0 }, { id: 2, name: "Course B", difficulty: 5, weeklyHours: 0 }],
      assignments: [
        { id: 3001, courseId: 1, title: "Paper A (earlier, not forced)", dueDate: dueA, status: "not-started", estimatedHours: 10, weight: 10, type: "project" },
        { id: 3002, courseId: 2, title: "Paper B (later, forced)", dueDate: dueB, status: "not-started", estimatedHours: 10, weight: 10, type: "project", forced: true },
      ],
      exams: [],
    };
    const result = planHorizon(dateStrs, data, narrowGaps, {});
    const combinedShortfall = result.shortfalls.reduce((s, it) => s + it.shortfallHours, 0);
    expect(combinedShortfall).toBeGreaterThan(0); // sanity: genuinely capacity-constrained
    const projectB = result.summaryItems.find((it) => it.id === "a_3002");
    expect(projectB.fullyCovered).toBe(true); // the forced project, due LATER, is still fully covered
  });
});
